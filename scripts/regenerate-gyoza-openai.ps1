...
$targetCodes = @(
  "G1", # GYOZA POULET LEGUMES
  "G2", # GYOZA BOEUF LEGUMES
  "G3"  # GYOZA LEGUMES
)
...
$backupDir = Join-Path $imagesDir "_backup-openai-gyoza-$stamp"
...
function Prompt-Base {
  return @(
    ...
    "Exactly one single gyoza only in the image.",
    "The gyoza must have a visible opening/cut so the filling is clearly seen.",
    "Do not split into two separate pieces.",
...
function Build-Prompt {
  ...
  switch ($Code) {
    "G1" {
      return "$base Dish: GYOZA POULET LEGUMES. Filling must clearly show minced chicken with vegetables (cabbage and carrot). Chicken color should be light beige."
    }
    "G2" {
      return "$base Dish: GYOZA BOEUF LEGUMES. Filling must clearly show minced beef with vegetables (cabbage and onion). Beef color should be darker brown than chicken."
    }
    "G3" {
      return "$base Dish: GYOZA LEGUMES. Vegetarian filling only: cabbage, carrot, mushroom and peas. No meat at all."
    }
    default {
