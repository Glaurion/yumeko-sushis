...
$backupDir = Join-Path $imagesDir "_backup-openai-m5-c9-$stamp"
...
function Build-Prompt {
  ...
  switch ($Code) {
    "M5" {
      return "$base Dish: MAKI CONCOMBRE CHEESE. EXACTLY 4 maki pieces, no more no less. Single straight row, evenly spaced, all lying down in the same direction. Nori outside, white rice, cucumber and cream-cheese center."
    }
    "C9" {
      return "$base Dish: CALIFORNIA SAUMON CHEESE OIGNONS FRITS. EXACTLY 4 pieces, no more no less. Single straight row, evenly spaced, all lying down in the same direction. Filling must contain salmon and cream cheese only. No avocado and no cucumber. Outer coating must be crispy fried onions only. No black sesame and no white sesame."
    }
    default {
