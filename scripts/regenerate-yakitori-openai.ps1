...
$backupDir = Join-Path $imagesDir "_backup-openai-yakitori-$stamp"
...
function Prompt-Base {
  return @(
    ...
    "Exactly one single yakitori skewer only in the image.",
...
function Build-Prompt {
  ...
  switch ($Code) {
    "BR1" {
      return "$base Dish: YAKITORI BOEUF FROMAGE. One skewer only, with beef carpaccio wrapped around melted emmental cubes. Cheese must be clearly visible. Char-grilled look and glossy yakitori glaze."
    }
    default {
