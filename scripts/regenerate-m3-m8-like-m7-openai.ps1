...
$backupDir = Join-Path $imagesDir "_backup-openai-m3-m8-like-m7-$stamp"
...
function Prompt-Base {
  return @(
    ...
    "Visual style must match MAKI AVOCAT CHEESE (M7) reference exactly.",
    "Same plain sakura pink background, same beige rectangular ceramic plate, same soft studio lighting, same 3/4 front camera angle, same clean framing and distance.",
    "Exactly 4 maki pieces.",
    "All 4 pieces upright, front faces visible, aligned in one straight row with identical spacing.",
...
function Build-Prompt {
  ...
  switch ($Code) {
    "M8" {
      return "$base Dish: MAKI CHEESE. Exactly 4 pieces only. Nori outside, white rice, cream cheese center only."
    }
    "M3" {
      return "$base Dish: MAKI SAUMON CHEESE. Exactly 4 pieces only. Nori outside, white rice, salmon and cream cheese filling."
    }
    default {
