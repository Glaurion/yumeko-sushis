...
$backupDir = Join-Path $imagesDir "_backup-openai-like-m1-$stamp"
...
function Prompt-Base {
  return @(
    ...
    "Visual style must match MAKI SAUMON (M1) reference: same plate size, same framing, same light, same 3/4 front camera angle.",
    "Small elegant ceramic plate.",
    "Exactly 4 pieces.",
    "All 4 pieces lying down, same direction and aligned cleanly.",
...
function Build-Prompt {
  ...
  switch ($Code) {
    "M2" {
      return "$base Dish: MAKI SAUMON AVOCAT. Nori outside, salmon and avocado filling."
    }
    "M3" {
      return "$base Dish: MAKI SAUMON CHEESE. Nori outside, salmon and cream-cheese filling."
    }
    "P3" {
      return "$base Dish: MAKI PRINTEMPS CONCOMBRE CHEESE. Structure: rice wrapped by rice paper then lettuce outside, filling cucumber and cream cheese."
    }
    "P1" {
      return "$base Dish: MAKI PRINTEMPS SAUMON AVOCAT. Structure: rice wrapped by rice paper then lettuce outside, filling salmon and avocado."
    }
    "R1" {
      return "$base Dish: ROLLS SAUMON AVOCAT CHEESE. Salmon visible as topping/outside, avocado and cream cheese in the center."
    }
    default {
